...
(async (): Promise<void> => {
	...
	server.onSocketOpen = async (socket: LiBaseSocket<PlaygroundClientCommands, PlaygroundServerCommands>): Promise<void> => {

		for (const s of server.getSockets()) {

			if (s.getId() === socket.getId()) continue;
			await s.invoke("newSibling", socket.getId());
